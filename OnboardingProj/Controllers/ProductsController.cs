...
namespace OnboardingProj.Controllers
{
    ...
    {
        private OnboardingDBEntities db = new OnboardingDBEntities();
        ...
        public ActionResult Create([Bind(Include = "Id,Name,Price")] Product product)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = (from x in db.Products where x.Id == NewId select x).ToList();

                if (user.Count > 0)
                {
                    ViewBag.Duplicate = "This Id already exists in the database. Please select another.";
                }
                else
                {
